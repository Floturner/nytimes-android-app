...
public class PoliticsFragment extends Fragment {
    ...
    @BindView(R.id.politics_swipe_refresh)
    SwipeRefreshLayout swipeRefreshLayout;
    @BindView(R.id.politics_recycler_view)
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        context = getActivity();
        if (context != null) {
            context.setTitle(R.string.nav_politics);
        }
    }
    ...
    void getArticles() {
        ...
        new Handler().postDelayed(new Runnable() {
            @Override
            public void run() {
                MainActivity.apiService.getStoriesBySection(getResources().getStringArray(R.array.sections)[2], APIClient.API_KEY).enqueue(callback);
            }
        }, 1500);
    }
