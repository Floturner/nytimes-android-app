...
public class SportsFragment extends Fragment {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        context = getActivity();
        if (context != null) {
            context.setTitle(R.string.nav_sports);
        }
    }
    ...
    void getArticles() {
        ...
        new Handler().postDelayed(new Runnable() {
            @Override
            public void run() {
                MainActivity.apiService.getStoriesBySection(getResources().getStringArray(R.array.sections)[4], APIClient.API_KEY).enqueue(callback);
            }
        }, 1500);
    }
