...
public class MoviesFragment extends Fragment {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        context = getActivity();
        if (context != null) {
            context.setTitle(R.string.nav_movies);
        }
    }
    ...
    void getArticles() {
        ...
        new Handler().postDelayed(new Runnable() {
            @Override
            public void run() {
                MainActivity.apiService.getStoriesBySection(getResources().getStringArray(R.array.sections)[1], APIClient.API_KEY).enqueue(callback);
            }
        }, 1500);
    }
